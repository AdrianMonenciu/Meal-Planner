...
export default function UpdateFood(mealItemProps){
    ...
    const initialValues: FormValues = {
        ...
        privateBool: mealItemProps.response.status == 'loaded' ? mealItemProps.response.payload.results[0].privateBool : false,
                                    ...
                                    {formik.values.image ?
                                        ...
                                        : <>
                                            ...
                                            <Image
                                                className={`${styles.avatar} `}
                                                cloudName={process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}
                                                publicId={mealItemProps.response.status == 'loaded' ? mealItemProps.response.payload.results[0].image : ""}
                                                alt={mealItemProps.response.status == 'loaded' ? mealItemProps.response.payload.results[0].image : ""}
                                                secure
                                                dpr="auto"
                                                quality="auto"
                                                //width={576}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    const options = {
      ...
      await fetch(`${url}/api/meal/getMeals?mealName=${mealNameQuerry}&limit=1&username=${sessionObj.user.username}`, options)
      .then(async (response) => {
        if (!response.ok) {
          const error = await response.json()
          //console.log(error)
