...
export default function UpdateFood(mealItemProps) {
  ...
  const initialValues: FormValues = {
    ...
    privateBool:
      mealItemProps.response.status == 'loaded' ? mealItemProps.response.payload.results[0].privateBool : false,
    allPrivateFoodItems:
      mealItemProps.response.status == 'loaded' ? mealItemProps.response.payload.results[0].privateAllFoods : false,
    image: null
  };
  ...
  const initialValues2: FormValues2 = {
    foodItems:
      mealItemProps.response.status == 'loaded'
        ? mealItemProps.response.payload.results[0].foodItems.map(foodItem => ({
                    ...
                    {dietPreferencesFood.map(diet => (
                      <label
                        className={`mr-3 ml-3 mt-1 ${
                          !currentDietPlan.includes(diet) && formik.values.diet.indexOf(diet) > -1
                            ? 'text-rose-500'
                            : ''
                  ...
                  <div className=" mt-1 min-w-[120px] max-w-[200px] mx-auto mb-1">
                    <button
                      ...
                      <span className="px-1 md:px-2">
                        {dietPlanUpdated ? 'Food items Updated' : 'Reset Food Items'}
                      </span>
                    </button>
                  </div>
                </div>
                ...
                <div className={`${styles.input_group} flex-col w-full bg-green-100`}>
                  ...
                  <label
                    className={`mr-3 ml-3 mt-1 text-sm md:text-base 
                      ${
                        formik.values.privateBool == true && formik.values.privateBool != privateMeal
                          ? 'text-rose-500'
                          : ''
                      }`}>
                    <input
                            ...
                            {values.foodItems.map((name, index) => (
                              <div
                                ...
                                className={`flex flex-col  w-full ${
                                  index < values.foodItems.length - 1 ? 'mb-4' : ''
                                }`}>
                                  ...
                                  <div className="w-16 h-16 ml-1 md:ml-2 flex items-center justify-center">
                                    {values.foodItems[index].image ? (
                                      <Image
                                        ...
                                        alt={
                                          values.foodItems[index].image ? (values.foodItems[index].image as string) : ''
                                        }
                                        secure
                                        dpr="auto"
                                        quality="auto"
