...
export default function CreateWeeklyPlan(weeklyPlanProps){
    const [inputData, setInputData] = useState<Service<IWeklyPlan>>({status: 'loading'})
    ...
    useEffect(() => {
        if(weeklyPlanProps.response.status == 'error') {
            setInputData({ status: 'error', error: weeklyPlanProps.response.error})
        } else {
            setInputData({ status: 'loaded', payload: weeklyPlanProps.response.payload })
            console.log(weeklyPlanProps)
            setWeekNrOptions(generateWeekNrOptions(weeklyPlanProps.response.payload.weeklyPlanData.availableWeekNumbers, weeklyPlanProps.response.payload.weeklyPlanData.year))
        }
    },[])

    function getWeekNrOptionLabel(weekNr) {
        const selectedOption = weekNrOptions.find((option) => option.value === weekNr);
    ...
    const initialValues: FormValues = {
        ...
        weekNr: weekNrOptions ? weekNrOptions[0].value : 0, //weeklyPlanProps.response.payload.weeklyPlanData.availableWeekNumbers[0] : 0,
    ...
    const validationSchema = Yup.object().shape({
        mondayMealsBreakfast: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.mondayMealsLunch.length > 0 || this.parent.mondayMealsDinner.length > 0)
            );}
        ),
        mondayMealsLunch: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.mondayMealsBreakfast.length > 0 || this.parent.mondayMealsDinner.length > 0)
            );}
        ),
        mondayMealsDinner: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.mondayMealsBreakfast.length > 0 || this.parent.mondayMealsLunch.length > 0)
            );}
        ),
        mondaySnaks: Yup.array().required().min(1, 'Min 1 snack required!').max(10, 'Maximum 10 snacks are allowed!').of(
            Yup.object().shape({
                qty: Yup.number().typeError('Min qty is 0.1!').required('Min qty is 0.1!').min(0.1, 'Min qty is 0.1!')
            })
        ),
        tuesdayMealsBreakfast: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.tuesdayMealsLunch.length > 0 || this.parent.tuesdayMealsDinner.length > 0)
            );}
        ),
        tuesdayMealsLunch: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.tuesdayMealsBreakfast.length > 0 || this.parent.tuesdayMealsDinner.length > 0)
            );}
        ),
        tuesdayMealsDinner: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.tuesdayMealsBreakfast.length > 0 || this.parent.tuesdayMealsLunch.length > 0)
            );}
        ),
        tuesdaySnaks: Yup.array().required().min(1, 'Min 1 snack required!').max(10, 'Maximum 10 snacks are allowed!').of(
            Yup.object().shape({
                qty: Yup.number().typeError('Min qty is 0.1!').required().min(0.1, 'Min qty is 0.1!')
            })
        ),
        wednesdayMealsBreakfast: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.wednesdayMealsLunch.length > 0 || this.parent.wednesdayMealsDinner.length > 0)
            );}
        ),
        wednesdayMealsLunch: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.wednesdayMealsBreakfast.length > 0 || this.parent.wednesdayMealsDinner.length > 0)
            );}
        ),
        wednesdayMealsDinner: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.wednesdayMealsBreakfast.length > 0 || this.parent.wednesdayMealsLunch.length > 0)
            );}
        ),
        wednesdaySnaks: Yup.array().required().min(1, 'Min 1 snack required!').max(10, 'Maximum 10 snacks are allowed!').of(
            Yup.object().shape({
                qty: Yup.number().typeError('Min qty is 0.1!').required().min(0.1, 'Min qty is 0.1!')
            })
        ),
        thursdayMealsBreakfast: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.thursdayMealsLunch.length > 0 || this.parent.thursdayMealsDinner.length > 0)
            );}
        ),
        thursdayMealsLunch: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.thursdayMealsBreakfast.length > 0 || this.parent.thursdayMealsDinner.length > 0)
            );}
        ),
        thursdayMealsDinner: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.thursdayMealsBreakfast.length > 0 || this.parent.thursdayMealsLunch.length > 0)
            );}
        ),
        thursdaySnaks: Yup.array().required().min(1, 'Min 1 snack required!').max(10, 'Maximum 10 snacks are allowed!').of(
            Yup.object().shape({
                qty: Yup.number().typeError('Min qty is 0.1!').required().min(0.1, 'Min qty is 0.1!')
            })
        ),
        fridayMealsBreakfast: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.fridayMealsLunch.length > 0 || this.parent.fridayMealsDinner.length > 0)
            );}
        ),
        fridayMealsLunch: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.fridayMealsBreakfast.length > 0 || this.parent.fridayMealsDinner.length > 0)
            );}
        ),
        fridayMealsDinner: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.fridayMealsBreakfast.length > 0 || this.parent.fridayMealsLunch.length > 0)
            );}
        ),
        fridaySnaks: Yup.array().required().min(1, 'Min 1 snack required!').max(10, 'Maximum 10 snacks are allowed!').of(
            Yup.object().shape({
                qty: Yup.number().typeError('Min qty is 0.1!').required().min(0.1, 'Min qty is 0.1!')
            })
        ),
        saturdayMealsBreakfast: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.saturdayMealsLunch.length > 0 || this.parent.saturdayMealsDinner.length > 0)
            );}
        ),
        saturdayMealsLunch: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.saturdayMealsBreakfast.length > 0 || this.parent.saturdayMealsDinner.length > 0)
            );}
        ),
        saturdayMealsDinner: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.saturdayMealsBreakfast.length > 0 || this.parent.saturdayMealsLunch.length > 0)
            );}
        ),
        saturdaySnaks: Yup.array().required().min(1, 'Min 1 snack required!').max(10, 'Maximum 10 snacks are allowed!').of(
            Yup.object().shape({
                qty: Yup.number().typeError('Min qty is 0.1!').required().min(0.1, 'Min qty is 0.1!')
            })
        ),
        sundayMealsBreakfast: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.sundayMealsLunch.length > 0 || this.parent.sundayMealsDinner.length > 0)
            );}
        ),
        sundayMealsLunch: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.sundayMealsBreakfast.length > 0 || this.parent.sundayMealsDinner.length > 0)
            );}
        ),
        sundayMealsDinner: Yup.array().max(3, 'Maximum 3 meals are allowed!').test('one-meal','At least one meal is required for breakfast, lunch, or dinner',
            function (value) {return (
                value && (value.length > 0 || this.parent.sundayMealsBreakfast.length > 0 || this.parent.sundayMealsLunch.length > 0)
            );}
        ),
        sundaySnaks: Yup.array().required().min(1, 'Min 1 snack required!').max(10, 'Maximum 10 snacks are allowed!').of(
            Yup.object().shape({
                qty: Yup.number().typeError('Min qty is 0.1!').required().min(0.1, 'Min qty is 0.1!')
            })
        ),
          ...
          if (day.endsWith("Snaks")) {
            ...
            for (const snak of snaks) {
              ...
              if (existingItem) {
                ...
                shoppingList.push({ foodItem: snak.name, qty: snak.qty, qtyOption: snak.qtyOption, isPurchased: false, image: snak.image });
              }
            }
          }
        }
              ...
              if (meal) {
                for (const foodItem of meal.foodItems) {
                  ...
                  if (existingItem) {
                    ...
                    shoppingList.push({ foodItem: foodItem.foodId.name, qty: foodItem.qty, qtyOption: foodItem.foodId.foodMeasureUnit, 
                        isPurchased: false, image: foodItem.foodId.image });
                  }
                }
              }
            }
    ...
    return (
        ...
        <Layout>
            ...
            <section className='w-[320px] md:w-[750px] mx-auto  items-center gap-3 mt-4 md:mt-8'> 
                ...
                {inputData.status === 'loading' && <div>Loading...</div>}
                {inputData.status === 'loaded' &&  (
                    <Formik
                    innerRef={formikRef}
                    ...
                    {({ values, handleSubmit, handleChange, errors, touched, resetForm,  setFieldValue, isSubmitting}) => (
                        ...
                        <Form className='w-[320px] md:w-[750px] grid grid-rows-10 mx-auto justify-items-center gap-3 mt-4 md:mt-8' onSubmit={handleSubmit}>
                            <div className='w-[320px] md:w-[750px] mb-2 grid grid-cols-1 md:grid-cols-2 md:grid-rows-1 md:gap-x-10 gap-5 justify-around'>
                                <div className='flex flex-col gap-2 md:col-start-1 md:row-start-1'>
                                    ...
                                    <Field name={`weekNr`}>
                                        ...
                                        <Select
                                            ...
                                            isSearchable={true}
                                            value={{ value: values.weekNr, label: getWeekNrOptionLabel(values.weekNr) }}
                                            //defaultValue={{ value: values.names[index].name, label: values.names[index].name }}
                                            options={weekNrOptions
                                ...
                                <div className={`${styles.input_group} flex-col md:col-start-2 md:row-start-1 bg-green-100`}>
                                    ...
                                    ${values.privateAll ? "All private food items and meals" : "Private food items and miels based on diet"}`}</div>
                                    <label className={`mr-3 ml-3 mt-1 text-sm md:text-base 
                                    ${(privateAll == true && values.privateAll != privateAll) ? 'text-rose-500' : ''}`}>
                                        <input type="radio" name="privateBool" className='mr-1' checked={privateAll === true}
                                    ...
                                    ${(privateAll == false && values.privateAll != privateAll) ? 'text-rose-500' : ''}`}>
                                        <input type="radio" name="privateBool" className='mr-1' checked={privateAll === false}
                                    ...
                                    <div className=" mt-1 min-w-[120px] max-w-[250px] mx-auto mb-1">
                                        <button disabled={privateAll == values.privateAll}
                                        ...
                                        onClick={() => resetPrivateAll(setFieldValue)}>
                                            ...
                                            {privateAll == values.privateAll ? 'Snacks/Meals Updated' : 'Reset Snacks/Meals'}</span>
                                        </button>
                                    </div>
                            ...
                            <div className={`w-[320px] md:w-[750px] ${daySelect == "wednesday" ? "block" : "hidden"}`}>
                                <DailyInputFieldArray values={values} weeklyPlanProps={weeklyPlanProps} 
                                setFieldValue={setFieldValue} fieldName="wednesday" errors={errors} touched={touched}/>
                            </div>

                            <div className={`w-[320px] md:w-[750px] ${daySelect == "thursday" ? "block" : "hidden"}`}>
                                <DailyInputFieldArray values={values} weeklyPlanProps={weeklyPlanProps} 
                                setFieldValue={setFieldValue} fieldName="thursday" errors={errors} touched={touched}/>
                            </div>

                            <div className={`w-[320px] md:w-[750px] ${daySelect == "friday" ? "block" : "hidden"}`}>
                                <DailyInputFieldArray values={values} weeklyPlanProps={weeklyPlanProps} 
                                setFieldValue={setFieldValue} fieldName="friday" errors={errors} touched={touched}/>
                            </div>

                            <div className={`w-[320px] md:w-[750px] ${daySelect == "saturday" ? "block" : "hidden"}`}>
                                <DailyInputFieldArray values={values} weeklyPlanProps={weeklyPlanProps} 
                                setFieldValue={setFieldValue} fieldName="saturday" errors={errors} touched={touched}/>
                            </div>

                            <div className={`w-[320px] md:w-[750px] ${daySelect == "sunday" ? "block" : "hidden"}`}>
                                <DailyInputFieldArray values={values} weeklyPlanProps={weeklyPlanProps} 
                                setFieldValue={setFieldValue} fieldName="sunday" errors={errors} touched={touched}/>
                            </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    async function getMeals(querryData: string[], privateAll: boolean, username: string) {
        ...
        //fetch(`/api/meal/getMeals?mealName=${querryData.mealName}&limit=20&username=${session.user.username}`, options)
    ...
    if (!userDietPreference) {
        ...
        const foodNamesAllPrivate = responseLoaded.payload.weeklyPlanData.availableFoodItemsAllPrivate != undefined ? 
        responseLoaded.payload.weeklyPlanData.availableFoodItemsAllPrivate.map(item => item.name) : [""];
        responseLoaded.payload.weeklyPlanData.foodItemsSelectOptionsAllPrivate = foodNamesAllPrivate.map((food) => ({value : food, label : food}))
        ...
        const mealNamesAllPrivate = responseLoaded.payload.weeklyPlanData.availableMealsAllPrivate != undefined ? 
        responseLoaded.payload.weeklyPlanData.availableMealsAllPrivate.map(item => item.name) : [""];
        responseLoaded.payload.weeklyPlanData.mealsSelectOptionsAllPrivate = mealNamesAllPrivate.map((meal) => ({value : meal, label : meal}))
    }
...
//         <section className='w-[700px] mx-auto  items-center gap-3 mt-4 md:mt-8'> 
//             <div className="flex justify-center">
//                 <h1 className='font-bold md:text-xl'>Weekly Plan</h1>
//             </div>
...
//                     <Form className='w-[750px] grid grid-rows-10 mx-auto justify-items-center gap-3 mt-4 md:mt-8' onSubmit={handleSubmit}>
//                         <div className='w-[600px] grid grid-cols-1 md:grid-cols-2 md:grid-rows-1 md:gap-x-10 gap-5 justify-around'>
//                             <div className='w-[300px] flex flex-col gap-2 md:col-start-1 md:row-start-1'>
//                                 <p className={`text-left ml-1 mt-1 text-sm md:text-base mb-1 font-medium`}>Week number:</p>
//                                 <Field name={`weekNr`}>
//                                     {({ field, form }) => (
//                                     <Select
