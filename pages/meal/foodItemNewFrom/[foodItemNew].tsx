...
export default function UpdateFood(foodItemProps) {
  ...
  const [foodItems, setFoodItems] = useState<Service<IFoodItems>>({
    status: "loading",
  });
  ...
  useEffect(() => {
    if (foodItemProps.response.status == "error") {
      ...
      setFoodItems({
        status: "loaded",
        payload: foodItemProps.response.payload,
      });
    }
  }, []);
  ...
  const initialValues: FormValues = {
    name:
      foodItemProps.response.status == "loaded"
        ? foodItemProps.response.payload.results[0].name
        : "",
    privateBool:
      foodItemProps.response.status == "loaded"
        ? foodItemProps.response.payload.results[0].privateBool
        : false,
    foodMeasureUnit:
      foodItemProps.response.status == "loaded"
        ? foodItemProps.response.payload.results[0].foodMeasureUnit
        : "",
    diet:
      foodItemProps.response.status == "loaded"
        ? foodItemProps.response.payload.results[0].diet
        : [],
    snack:
      foodItemProps.response.status == "loaded"
        ? foodItemProps.response.payload.results[0].snack
        : false,
    image: null,
  };
  ...
  return (
    <Layout>
      ...
      <section className="min-w-[250px] max-w-[320px] md:max-w-[900px] items-center mx-auto flex flex-col gap-3 mt-4 md:mt-8">
            ...
            <div className="flex flex-col md:flex-row md:gap-x-10 gap-5">
              <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col">
                  <div
                    className={`${styles.input_group} ${
                      formik.values.foodMeasureUnit == "" && formik.touched.name
                        ? "border-rose-600"
                        : ""
                    }`}
                  >
                    <select
                      name="dietPreference"
                      value={formik.values.foodMeasureUnit}
                      className={styles.input_text}
                      {...formik.getFieldProps("foodMeasureUnit")}
                    >
                      {
                        <option value="" disabled={true}>
                          Select a food measuring unit
                        </option>
                      }
                      {foodMeasureUnit.map((unit) => (
                  ...
                  {formik.errors.foodMeasureUnit &&
                  formik.touched.foodMeasureUnit ? (
                    <span className={`${styles.formikError}`}>
                      {formik.errors.foodMeasureUnit}
                    </span>
                  ) : (
                    ...
                    {dietPreferencesFood.map((diet) => (
                      ...
                      >
                        <input
                          ...
                          value={diet}
                          defaultChecked={
                            foodItems.payload.results[0].diet.indexOf(diet) > -1
                              ? true
                              : false
                          }
                        />
              ...
              <div className="flex flex-col items-center gap-5">
                <div
                  className={`${styles.input_group} w-full flex-col bg-green-100`}
                >
                ...
                <div className={`${styles.input_group} flex-col bg-green-100`}>
                  <label className="mr-3 ml-3 mt-1 text-sm md:text-base">
                    <input
                      ...
                      name="snack"
                      className="mr-1"
                      checked={formik.values.snack === false}
                  ...
                  <label className="mr-3 ml-3 mt-1 mb-1 text-sm md:text-base">
                    <input
                      ...
                      name="snack"
                      className="mr-1"
                      checked={formik.values.snack === true}
                ...
                <div className="flex flex-col">
                  <label
                    htmlFor="image"
                    className={`${styles.button} min-w-[150px] max-w-[200px] text-center`}
                  >
                    Choose File
                  </label>
                  <input
                  ...
                  {formik.errors.image && formik.touched.image ? (
                    <span className="text-rose-500 mt-1 ml-1 text-sm md:text-base">
                      {formik.errors.image}
                    </span>
                  ) : (
                    <></>
                  ...
                  {formik.values.image ? (
                    ...
                    <>
                      ...
                      <Image
                        ...
                        publicId={
                          foodItemProps.response.status == "loaded"
                            ? foodItemProps.response.payload.results[0].image
                            : ""
                        }
                        alt={
                          foodItemProps.response.status == "loaded"
        ...
        {foodItems.status === "error" && <div>{foodItems.error}</div>}
      </section>
    </Layout>
  );
}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const response = await fetch(
    `${url}/api/meal/getFoodItems?foodName=${foodNameQuerry}&limit=1&username=${sessionObj.user.username}`,
    options
  );
